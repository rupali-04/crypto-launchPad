...
const VentureProject = () => {
    ...
    return(<><div>
       ...
       {isLoggedIn === false ? "" : <><Flex mt="2rem"  h="max-content" ml="20rem"><Card bg="gray.100" borderRadius={"40px"} w="max-content" h="max-content"><CardHeader><Flex mt="1rem" gap="1"><Text>NO OF BID: 15 </Text><Text> | </Text><Text>ACCEPT BID: 2</Text><IconButton mt="-0.5rem" aria-label="icon" icon={<AiOutlineQuestionCircle/>}/></Flex> </CardHeader></Card></Flex>
       <Flex mt="8rem"  ml="-20.5rem"><Card h="max-content" bg="gray.100" borderRadius={"40px"}><CardBody><Text textAlign={"center"} fontWeight="bold" fontSize={"22px"}><IconButton aria-label='Search database' icon={<BsWallet2 />} cursor="none" pl="0.25rem" h="max-content"  fontSize={"24"} variant={"unstyled"}></IconButton>Balance : 10000 BUSD</Text><Flex> <Input bg="white" borderRadius={"0"} placeholder="BIDDING PRICE" _placeholder={{ fontSize: '8px' }} borderLeft="black" w="100px" variant='outline' /><Input bg="white" placeholder="BIDDING AMOUNT" _placeholder={{ fontSize: '8px' }} borderRadius={"0"} w="100px" variant='outline' /><Button borderRadius={"0"} colorScheme={"blue"}>PLACE BID</Button></Flex><Text textAlign={"center"} color="gray" fontSize={"12px"}>T&C: Bid vaild only for 14 working days*</Text></CardBody></Card></Flex>
       </>}
       </Flex>
       <Box ml="2rem" mt="3rem">
        <Flex gap={"10"} >
